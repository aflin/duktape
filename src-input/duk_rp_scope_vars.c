 ...
 *  Rampart contribution (post duktape 2.7.0).  See util/rp_config.h.
 *
 *  Touches duktape internal types (duk_hdecenv, duk_hobjenv,
 *  duk_hcompfunc, duk_activation) — needs the amalgamation include of
 *  duk_internal.h to see them.
 *
 *  Two modes:
 *
 *  1) varname == NULL:  Push a plain object { name: value, ... } containing
 *     all variables belonging to the scope 'type' (DUK_RP_SCOPE_LOCAL, etc.).
 *     For DUK_RP_SCOPE_CLOSURE, multiple parent declarative envs are merged
 *     (inner scope wins on name collisions).
 *     For DUK_RP_SCOPE_WITH / DUK_RP_SCOPE_GLOBAL, the target object itself
 *     is pushed (not a copy).
 *
...
/* The SCOPE constants are also declared in duktape.h.in under the same
 * flag so callers from outside duktape's amalgamation can use them.
 * Definitions kept here for the local body's use. */
#define DUK_SCOPE_LOCAL   DUK_RP_SCOPE_LOCAL
#define DUK_SCOPE_CLOSURE DUK_RP_SCOPE_CLOSURE
#define DUK_SCOPE_WITH    DUK_RP_SCOPE_WITH
#define DUK_SCOPE_GLOBAL  DUK_RP_SCOPE_GLOBAL
