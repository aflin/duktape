...
/* Rampart vendored copy.  Prefix every public mp_* / s_mp_* function
 * and global with duk_rp_ so the symbols emitted into duktape's
 * amalgamation cannot collide with another libtommath an embedder
 * might link in (rampart-crypto via OpenSSL etc.).  Struct/type names
 * are intentionally NOT renamed -- they remain visible to the
 * consumer (duk_rp_bigint.c) under their canonical names. */
#include "tommath_rename.h"
