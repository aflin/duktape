...
/* Rampart vendored copy.  tommath_class.h is REPLACED at configure.py
 * time with a flat "define every kept BN_*_C" header -- libtommath's
 * stock 3-pass dependency-resolution mechanism uses a recursive
 * self-include that combine_src.py treats as a duplicate.  We already
 * pruned the .c file set manually, so flat-define is equivalent.
 * tommath_superclass.h is unused by this build and never included. */
#include "tommath.h"
