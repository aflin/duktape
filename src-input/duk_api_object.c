...
/* Rampart contribution: lightweight "is this an own property and what
 * are its attribute flags?" check.  Same information as
 * duk_get_prop_desc would put on an allocated descriptor object, but
 * without the per-call allocation -- reads duk_propdesc directly from
 * object internals.  Returns 1 if the key is an OWN property of
 * obj_idx, 0 otherwise.  On a hit, *out_flags receives a bitmask of
 * DUK_PROPATTR_* flags.  The key (at stack top) is popped, matching
...
#endif  /* DUK_RP_USE_OWN_PROP_INFO */
