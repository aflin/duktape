...
/* Define a prototype method as a non-enumerable, writable, configurable
 * data property holding a freshly-pushed C function -- the attribute
 * shape ECMAScript uses for all built-in prototype methods.  Crucially
 * non-enumerable, so the methods don't leak into `for (k in mapInstance)`
 * (real Set/Map instances iterate to nothing; clone-deep / kind-of and
 * Babel iterable helpers depend on that).  Object at proto_idx. */
static void rp__map_def_method(duk_context *ctx, duk_idx_t proto_idx,
...
/* Push the well-known symbol Symbol.<name> onto the stack.  Returns 1 if
 * pushed (leaving the symbol on top), 0 if Symbol or the well-known
 * symbol is unavailable (stack unchanged). */
static duk_bool_t rp__push_wk_symbol(duk_context *ctx, const char *name)
{
    duk_get_global_string(ctx, "Symbol");
    if (duk_is_undefined(ctx, -1)) { duk_pop(ctx); return 0; }
    duk_get_prop_string(ctx, -1, name);
    if (duk_is_undefined(ctx, -1)) { duk_pop_2(ctx); return 0; }
    duk_remove(ctx, -2);  /* drop Symbol, leave Symbol.<name> on top */
    return 1;
}

/* Apply the shared identity surface to a constructor/prototype pair that
 * is currently on the stack as [ ..., ctor, proto ] (proto on top):
 *   - ctor.name = tag                         (configurable)
 *   - proto.constructor = ctor                (non-enum, writable, configurable)
 *   - proto[Symbol.toStringTag] = tag         (non-enum, configurable)
 *   - proto[Symbol.iterator] = iter_fn        (non-enum, writable, configurable)
 * Leaves the stack as it found it ([ ..., ctor, proto ]). */
static void rp__map_finish_class(duk_context *ctx, const char *tag,
                                 duk_c_function iter_fn)
{
    duk_idx_t proto_idx = duk_normalize_index(ctx, -1);
    duk_idx_t ctor_idx  = duk_normalize_index(ctx, -2);

    /* ctor.name = tag */
    duk_push_string(ctx, "name");
    duk_push_string(ctx, tag);
    duk_def_prop(ctx, ctor_idx, DUK_DEFPROP_HAVE_VALUE |
                                DUK_DEFPROP_HAVE_CONFIGURABLE |
                                DUK_DEFPROP_SET_CONFIGURABLE |
                                DUK_DEFPROP_FORCE);

    /* proto.constructor = ctor (non-enumerable per spec) */
    duk_push_string(ctx, "constructor");
    duk_dup(ctx, ctor_idx);
    duk_def_prop(ctx, proto_idx, DUK_DEFPROP_HAVE_VALUE |
                                 DUK_DEFPROP_HAVE_WRITABLE | DUK_DEFPROP_SET_WRITABLE |
                                 DUK_DEFPROP_HAVE_CONFIGURABLE | DUK_DEFPROP_SET_CONFIGURABLE);

    /* proto[Symbol.toStringTag] = tag (non-enumerable, configurable, not writable) */
    if (rp__push_wk_symbol(ctx, "toStringTag")) {
        duk_push_string(ctx, tag);
        duk_def_prop(ctx, proto_idx, DUK_DEFPROP_HAVE_VALUE |
                                     DUK_DEFPROP_HAVE_CONFIGURABLE |
                                     DUK_DEFPROP_SET_CONFIGURABLE);
    ...
    /* proto[Symbol.iterator] = iter_fn (non-enumerable) */
    if (rp__push_wk_symbol(ctx, "iterator")) {
        duk_push_c_function(ctx, iter_fn, 0);
        duk_def_prop(ctx, proto_idx, DUK_DEFPROP_HAVE_VALUE |
                                     DUK_DEFPROP_HAVE_WRITABLE | DUK_DEFPROP_SET_WRITABLE |
                                     DUK_DEFPROP_HAVE_CONFIGURABLE | DUK_DEFPROP_SET_CONFIGURABLE);
    }
...
DUK_INTERNAL void duk_rp_install_map_set(duk_context *ctx)
{
    ...
    rp__map_def_method(ctx, -1, "set",     map_set,     2);
    rp__map_def_method(ctx, -1, "get",     map_get,     1);
    rp__map_def_method(ctx, -1, "has",     map_has,     1);
    rp__map_def_method(ctx, -1, "delete",  map_delete,  1);
    rp__map_def_method(ctx, -1, "clear",   map_clear,   0);
    rp__map_def_method(ctx, -1, "forEach", map_forEach, DUK_VARARGS);
    rp__map_def_method(ctx, -1, "entries", map_entries, 0);
    rp__map_def_method(ctx, -1, "keys",    map_keys,    0);
    rp__map_def_method(ctx, -1, "values",  map_values,  0);

    /* size getter (non-enumerable, configurable) */
    duk_push_string(ctx, "size");
    duk_push_c_function(ctx, map_size_getter, 0);
    duk_def_prop(ctx, -3, DUK_DEFPROP_HAVE_GETTER |
                           DUK_DEFPROP_SET_CONFIGURABLE);

    /* name / constructor / Symbol.toStringTag / Symbol.iterator(=entries) */
    rp__map_finish_class(ctx, "Map", map_entries);
    ...
    rp__map_def_method(ctx, -1, "add",     set_add,     1);
    rp__map_def_method(ctx, -1, "has",     map_has,     1);
    rp__map_def_method(ctx, -1, "delete",  map_delete,  1);
    rp__map_def_method(ctx, -1, "clear",   map_clear,   0);
    rp__map_def_method(ctx, -1, "forEach", set_forEach, DUK_VARARGS);
    /* Set stores values in .k field, so use map_keys (kind=1) for values */
    rp__map_def_method(ctx, -1, "values",  map_keys,    0);
    rp__map_def_method(ctx, -1, "keys",    map_keys,    0);
    rp__map_def_method(ctx, -1, "entries", set_entries, 0);
    ...
    /* name / constructor / Symbol.toStringTag / Symbol.iterator(=values) */
    rp__map_finish_class(ctx, "Set", map_keys);
